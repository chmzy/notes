...
### Визуально
```
nums[i] + nums[j] + nums[k] == 0   Нужно найти все тройки чисел, сумма которых 
nums[i] + nums[j] = -nums[k]       равно нулю.

Сортируем массив. Идем в цикле по nums, для каждого числа нужно решить задачу TwoSum II: 
	1. nums[i] = target; 
	2. find nums[l]+nums[r]=-target
Пары могут повторяться, складываем результат в множество(set).
```
### Код
1. Сортируем массив
