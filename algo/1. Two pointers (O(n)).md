...
Ставим 2 указателя на начало и конец строки. Пока не встретились: 
1. проверяем является ли символ буквой или числом (a <= char <= z). Если нет - сдвигаем указатель и переходим к следующей итерации. 
2. Если буквы в нижнем регистре не одинаковы - возвращаем False
3. **СДВИГАЕМ УКАЗАТЕЛИ** на 1
4. В конце возвращаем True
```python
class Solution:
...
### Визуально
```
nums[i] + nums[j] + nums[k] == 0   Сортируем массив. Для каждого числа из nums - 
nums[i] + nums[j] = -nums[k]       нужно решить задачу TwoSum II: 
                                   nums[i] = target; find nums[l]+nums[r]=-target
                                   Пары могут повторяться, складывай рез. в set.
```
### Код
1. Сортируем массив 
2. Для каждого элемента ищем сумму, ему противоположную
3. Добавляем триплеты в set, чтобы убрать все дубликаты.
```python
class Solution:
...
Пока один из них не дойдет до конца своей последовательности, то сравниваем: если chars на указателях равны - значит нашли char из subseq в str, сдвигаем p_sub. Иначе сдвигаем p_str. В конце, p_sub будет равен длине sub, возвращаем.
...
### Визуально
```
По условию, нам даны 2 массива, отсортированых по неубывнию, и индексы, на которых стоят их последние элементы (помечены *). Первый массив-приемник всегда сможет вместить в себя второй массив. Заполняем больший с конца.

Ставим результирующий указатель (помечен ^) на конец первого массива n1.
Пока (*) указатели в пределах своих массивов(>=0), сравниваем числа под ними: большее мы записываем в конец n1, сдвигаем больший * и ^.
