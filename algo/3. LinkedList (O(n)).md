...
# Алгоритм решения
* Если в ограничениях задачи кол-во элементов в списке
  может равняться нулю ИЛИ 
* Если при сортировке/слиянии двух список не понятно 
  какая нода станет головой ТО
...
1->2->2->1->N  Является ли список палиндромом.
               Для решения задачи, нужно:
				   1. Дойти до середины списк(slow.next, fast.next.next)
...
0. Palindrome не требует прерывания цикла, 
   ...
   Для изменения порядка в списке порвать цикл.

1. Рвем цикл: s = mid.next, mid.next = None; 
2. После переворачивания с second - 2 независ. списка
...
Алгоритм решения:
	...
	2. Используя алгоритм Флойда, ищем элемент, 
	   ...
	   выходим из бексконечного цикла:
		...
		2.3. slow1 == fast -> выходим из цикла
...
f  |x      |            d(1,5) = C - x
   +-5<-4<-+   Так как slow движется в 2 раза медленнее fast, 
			   ...
			   как только slow и fast встретятся:
                    |  2*slow1 = fast    |
				    | 2(p+C-x) = p+C+C-x |
					| 2p+2C-2x = p+2C-x  |
					|      p-x = 0       |
...
на начало цикла в списке, так как пройду одинаковое расстояние.

В общем случае, расстояние p может быть равно N. 
Тогда fast указателю придется пройти несколько раз по всему циклу, 
но, в конечном итоге, slow1 и fast встретятся за N шагов до начала цикла.
```
## Код
```python
class Solution:
...
5->6->4  342 это 2->4->3 и так далее. Нужно сложить эти два числа,
...
## Визуально
```
Список, у корого ноды содержат 3 поля:
1. Значение
2. Указатель на следующую ноду
3. Указатель на рандомную ноду

По условию, нужно сделать deep-copy списка, 
то есть полную копию и вернуть указатель на голову.
